...
#-------------------------------------------------------------------------------
def main(sub, dpath, spath_root, mpath, gt_name, flag, cerebrum_slice, max_iter_M3):
    #---------------------------------------------------------------------------
    ...
    if JE_flag and cnn_flag: #UNet + JE
        spath = spath_root + r'/w_cnn_combo_PE1_AP_CorrectMask_Rerun-06-24-2024'
        # max_loops = 250
        max_loops = max_iter_M3
    ...
    # spath_temp = spath_root + r'/w_cnn_combo_PE1_AP_CorrectMask_Rerun-06-19-2024' #path for initial run
...
if __name__ == "__main__":
    ...
    #
    # run_only = [2,3,4,5] #rerun only test cases 7, 9
    # max_iters = [100, 200, 100, 100]
    run_only = [6] #rerun only test cases 7, 9
    max_iters = [750]
    count = 0
    for i, paths in enumerate(paths_list):
        if i in run_only:
            ...
            for flag in flags:
                spath, m_corrupted, m_final, m_loss_store, Mtraj_store = main(sub, dpath, spath_root, mpath, gt_name, flag, cerebrum_slice, max_iters[count])
                count+=1
                xp.save(spath + r"/m_corrupted.npy", m_corrupted)
