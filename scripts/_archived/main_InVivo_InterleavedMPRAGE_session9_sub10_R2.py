...
def main(dpath, spath_root, mpath, gt_name):
    ...
    #---------------------------------------
    R_factor = 2
    U_init = np.load(dpath + r'/npy/samp_order.npy', allow_pickle=1) #LR, AP, SI
    U = U_init[::R_factor] #in this case, undersampling R=2 is equal to slicing every other  
    #Apply the undersampling
    U_array = xp.sum(xp.asarray([eop._gen_U_n(U[n], s_corrupted.shape[1:]) for n in range(len(U))]), axis = 0)
    s_corrupted *= U_array
    del U_array
    #---------------------------------------------------------------------------
    res = xp.array([1,1,1])
    ...
    #
    m_corrupted = rec.ImageRecon(A, b, m_init, mask = mask, maxiter=CG_maxiter, \
                        tol=CG_tol, atol=CG_atol)[-1]
    #
    #---------------------------------------
    ...
    if JE_flag and cnn_flag: #UNet + JE
        spath = spath_root + r'/npy/w_cnn_combo_PE1_AP_CorrectMask_R2'
        max_loops = 250
    elif JE_flag and not cnn_flag: #only JE
        spath = spath_root + r'/npy/wo_cnn_CorrectMask_R2'
        max_loops = 250
    elif not JE_flag and cnn_flag: #only UNet
        spath = spath_root + r'/npy/w_only_cnn_combo_PE1_AP_R2'
        max_loops = 1
