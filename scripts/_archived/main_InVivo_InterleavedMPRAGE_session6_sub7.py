...
#-------------------------------------------------------------------------------
def main(dpath, spath_root, mpath, gt_name, cerebrum_slice):
    #---------------------------------------------------------------------------
    ...
    C_init = xp.load(mpath + r'/{}/npy/sens.npy'.format(gt_name))
    # C_init = xp.load(dpath + r'/npy/sens.npy') #For Scan 4, because different head position
    ...
    # cerebrum_slice= 195 #Jacob Scan 3
    # cerebrum_slice= 215 #Jacob Scan 4 --> subject had shifted quite a bit between Scan 3 and 4
    ...
    if JE_flag and cnn_flag: #UNet + JE
        # spath = spath_root + r'/npy/w_cnn_combo_PE1_AP_CorrectMask_FixedStepSize'
        spath = spath_root + r'/npy/w_cnn_combo_PE1_AP_CorrectMask_250Iters_AltSlice215'
        # spath = spath_root + r'/npy/w_cnn_combo_PE1_AP_CorrectMask_CSPalt'
        # max_loops = 50
        max_loops = 250
    elif JE_flag and not cnn_flag: #only JE
        spath = spath_root + r'/npy/wo_cnn_CorrectMask_AltSlice215'
        # spath = spath_root + r'/npy/wo_cnn_CorrectMask_CSPalt'
        max_loops = 250
...
if __name__ == "__main__":
    ...
    # test_cases = ['scan3-InstructedMotion-16shots', 'scan4-FreeMotion-16shots']
    test_cases = ['scan3-InstructedMotion-16shots']
    # cerebrum_slices = [195, 215]
    cerebrum_slices = [215]
    for i, test_case in enumerate(test_cases):
        ...
        spath_root = dpath
        spath, m_corrupted, m_final, m_loss_store, Mtraj_store = main(dpath, spath_root, mpath, gt_name, cerebrum_slices[i])
        xp.save(spath + r"/m_corrupted.npy", m_corrupted)
