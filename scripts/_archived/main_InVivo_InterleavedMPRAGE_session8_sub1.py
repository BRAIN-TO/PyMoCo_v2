...
def main(dpath, spath_root, mpath, gt_name):
    ...
    #---------------------------------------------------------------------------
    # s_corrupted = xp.load(gt_name + r'/kdat_trunc.npy') #NC, SI, AP, LR
    # C_init = xp.load(gt_name + r'/sens_pad256.npy')
    # C = xp.transpose(C_init, (3,0,1,2))[:,16:-16,16:-16,:]; del C_init
    # mask = rec.getMask(C); xp.save(gt_name + r'/m_GT_brain_mask.npy', mask)
    # U = [[np.arange(s_corrupted.shape[1]), np.arange(s_corrupted.shape[2]), np.arange(s_corrupted.shape[3])]]
    # res = xp.array([1,1,1])
    # R_pad = (10, 10, 10)
    # batch = 1
    # Mtraj_init = xp.zeros((len(U), 6))
    # m_GT = eop.Encode_Adj(s_corrupted, C, U, Mtraj_init, res, batch=batch) #E.H*s
    # m_GT = xp.pad(m_GT, ((16,16), (16,16), (0,0)))
    # xp.save(gt_name + r'/img_CG.npy', m_GT)
    ...
    C_init = xp.load(dpath + r'/npy/sens.npy')
    ...
    res = xp.array([1,1,1])
    #---------------------------------------
    m_GT = xp.load(gt_name + r'/img_CG.npy')
    ...
    JE_flag = 0 #turn JE algorithm on / off
    thresh = {'severe': 500, 'moderate': 0.1}
    if JE_flag and cnn_flag: #UNet + JE
        spath = spath_root + r'/npy/w_cnn_combo_PE1_AP_CorrectMask_250Iters'
        max_loops = 250
    elif JE_flag and not cnn_flag: #only JE
        spath = spath_root + r'/npy/wo_cnn_CorrectMask'
        max_loops = 250
...
if __name__ == "__main__":
    subname = "Sub1"
    mpath = r'/home/nghiemb/Data/TWH/MPRAGE_PE1Reordered/Scan20231204/{}/h5data'.format(subname)
    test_case = 'scan1-ContinuousMotion-16shots'
    # test_case = 'scan2-ContinuousMotion-32shots'
    gt_name = r'/home/nghiemb/Data/TWH/MPRAGE_PE1Reordered/Scan20230710/scan1-reference-noreorder'
