...
def main(dpath, spath_root, mpath, gt_name):
    ...
    C_init = xp.load(mpath + r'/{}/npy/sens.npy'.format(gt_name))
    # C_init = xp.load(dpath + r'/npy/sens.npy') #For Scan 5, where I had turned off half the coils
...
if __name__ == "__main__":
    ...
    test_case = 'scan4-InstructedMotion-16shots'
    # test_case = 'scan5-FreeMotion-16shots' #NB. only acquired data from 10 coils for this scan***
    gt_name = 'scan2-ReferenceProduct-16shots-v2'
...
for i, ax in enumerate(axes.flatten()):
    ax.imshow(abs(C[i,128,:,:]), cmap = "gray", vmax = 1.0)
    ax.title.set_text('Coil {}'.format(i+1))


m_coils = eop._ifft(s_corrupted, (1,2,3))

fig, axes = plt.subplots(2,5)
for i, ax in enumerate(axes.flatten()):
    ax.imshow(abs(m_coils[i,128,:,:]), cmap = "gray")
    ax.title.set_text('Coil {}'.format(i+1))

A = partial(eop._EH_E, C=C, U=U, Mtraj=Mtraj_est, res=res, lamda = CG_lamda, batch=batch)
b = eop.Encode_Adj(s_corrupted, C, U, Mtraj_est, res, batch=batch)

m_out = rec.ImageRecon(A, b, m_init, mask = mask, maxiter=CG_maxiter, tol=CG_tol, atol=CG_atol)
m_temp = m_out[-1]
