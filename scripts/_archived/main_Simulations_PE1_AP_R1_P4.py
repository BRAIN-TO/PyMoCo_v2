...
def main(dpath, spath_root, mpath, gt_name, test_flag):
    ...
    U_init = eop._U_Array2List(U_array, m_GT.shape)
    #Up-sample to every PE1
    U_GT = eop._U_subdivide(U_init, TR_shot)
    #---------------------------------------------------------------------------
    ...
    sinusoid_ncycles = 20
    sinusoid_nshots = len(U_GT)
    sinsusoid_amplitude = 5
    Mtraj_shots = (2*xp.pi)*(sinusoid_ncycles / sinusoid_nshots)*xp.arange(0,len(U_GT))
    sinusoid_R_LR = sinsusoid_amplitude*xp.sin(Mtraj_shots)
    sinusoid_T_SI = sinusoid_R_LR / 2 #half the amplitude and reversed sign
    Mtraj_GT = xp.zeros((len(U_GT),6))
    Mtraj_GT = Mtraj_GT.at[:,4].set(-sinusoid_R_LR) #rotations about LR axis
    Mtraj_GT = Mtraj_GT.at[:,2].set(-sinusoid_T_SI) #translations along SI axis
    xp.save(dpath + r'/Mtraj.npy', Mtraj_GT)
    #
    s_corrupted = eop.Encode(m_GT, C, U_GT, Mtraj_GT, res, batch=batch)
    xp.save(dpath + r'/s_corrupted.npy', s_corrupted)
    #---------------------------------------------------------------------------
    #------------------JOINT IMAGE RECON AND MOTION ESTIMATION------------------
    #---------------------------------------------------------------------------
    #Initializing update vars
    U = U_init #set U to be the nominal sampling scheme, rather than the true upsampled version
    ...
    if JE_flag and cnn_flag: #UNet + JE
        spath = spath_root + r'/w_cnn_combo_PE1_AP_CorrectMask_Continuity'
        max_loops = 250
        # max_loops = 64
    ...
    # m_est = xp.load(spath + r'/m_intmd.npy')
    # m_loss_store = list(xp.load(spath + r'/m_loss_store.npy', allow_pickle=1))
    # Mtraj_store = list(xp.load(spath + r'/Mtraj_store.npy', allow_pickle=1))
...
if __name__ == "__main__":
    mpath = r'/home/nghiemb/PyMoCo/data/cc/test/PE1_AP/Complex/R1/Paradigm_4'
    cases = [1]
